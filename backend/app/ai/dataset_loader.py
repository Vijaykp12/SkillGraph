...
class SkillGraphDatasetLoader:
    ...
    async def load_heterodata(self) -> tuple[HeteroData, dict, dict]:
        ...
        Falls back to local static seed data if Neo4j is offline.
        """
        try:
            ...
            print("[Warning] Falling back to static seed data to build HeteroData object...")
            return self.load_heterodata_static()

    def load_heterodata_static(self) -> tuple[HeteroData, dict, dict]:
        """
        Constructs HeteroData directly from static seed taxonomy definitions.
        Useful when databases are offline (e.g. local compilation, precomputing).
        """
        try:
            from scripts.seed_data import SKILLS, OCCUPATIONS, EDGES
        except ImportError:
        ...
        req_edges = []
        req_weights = []
        for u, v, w in EDGES.get("REQUIRES", []):
            o_idx = occ_to_idx.get(u)
            s_idx = skill_to_idx.get(v)
            if o_idx is not None and s_idx is not None:
                req_edges.append([o_idx, s_idx])
                req_weights.append(w)
        ...
        sim_edges = []
        sim_weights = []
        for u, v, w in EDGES.get("SIMILAR_TO", []):
            s1_idx = skill_to_idx.get(u)
            s2_idx = skill_to_idx.get(v)
            if s1_idx is not None and s2_idx is not None:
                sim_edges.append([s1_idx, s2_idx])
        ...
        # LEADS_TO edges
        lead_edges = []
        for u, v in EDGES.get("LEADS_TO", []):
            o1_idx = occ_to_idx.get(u)
            o2_idx = occ_to_idx.get(v)
            if o1_idx is not None and o2_idx is not None:
                lead_edges.append([o1_idx, o2_idx])
